...
if ((session.getAttribute("sessao")=="F")||(null == session.getAttribute("sessao"))){response.sendRedirect("login.jsp");} %>
			...
			<img:Image patientId="${param.patient}" study="${param.study}" seriesId="${param.series}" >
				<tr><td>
					<div class="imgNo">Number of Frames : &nbsp;${numberOfFrames}</div>
				
							
						<img alt=""  src="Image.do?study=${param.study}&series=${param.series }&object=${imageId }" width="135" onclick=" setImageInfos('${numberOfFrames}'); initScroll(); setImage=true; cineloop=0; ajaxpage('imagePane','MultiFrames.jsp?study=${param.study}&series=${param.series }&object=${imageId }&numberOfFrames=${numberOfFrames }&sex=${param.sex }&physicianName=${param.physicianName }&birthDate=${param.birthDate }&studyDates=${param.studyDates }&modality=${param.modality }'); return false;">		
