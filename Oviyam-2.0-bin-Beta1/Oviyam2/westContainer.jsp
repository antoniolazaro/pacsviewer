            ...
            <img:Image patientId="${param.patient}" study="${param.study}" series="${seriesId}" dcmURL="${param.dcmURL}">
                <c:if test="${multiframe == 'yes'}">
                    <c:forEach var="i" begin="1" end="${numberOfFrames}">
                        <img name="${i}" id="${fn:replace(imageId, '.','_')}_${i}" src="/wado?requestType=WADO&studyUID=${param.study}&seriesUID=${seriesId}&objectUID=${imageId}&rows=512&frameNumber=${i}" height="48px" style="display:none" />
                    </c:forEach>
                </c:if>
            </img:Image>
            </td>
