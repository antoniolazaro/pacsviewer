...
<html>
    ...
    <body>
        <div id="patName" class="heading">${param.patientName}</div>
        ...
        <table id="studyTable" style="font-family: Arial,Helvitica,Serif; font-size:12px; width: 100%;border: 2px outset white; " >
            <tbody>
                <tr>
                    <td colspan="2">${param.studyDesc}</td>
                </tr>
                <tr>
            ...
            <img:Image patientId="${param.patient}" study="${param.study}" series="${seriesId}" dcmURL="${param.dcmURL}">
            	<c:choose>
                    <c:when test="${modality == 'SR'}">
                        <img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" src="images/SR_Latest.png" imgSrc="Image.do?serverURL=${param.wadoUrl}&study=${param.study}&series=${seriesId}&object=${imageId}&rows=512" height="48px" />
                    </c:when>

                    <c:when test="${sopClassUID == '1.2.840.10008.5.1.4.1.1.104.1'}">
                        <img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" src="images/pdf.png" imgSrc="Image.do?serverURL=${param.wadoUrl}&study=${param.study}&series=${seriesId}&object=${imageId}&rows=512" height="48px" />
                    </c:when>

                    <c:otherwise>
                        <c:if test="${(instanceNumber == middle) || (instanceNumber==1) || (instanceNumber==numberOfImages)}">
                            <img name="${instanceNumber}" id="${fn:replace(seriesId, '.','_')}_${instanceNumber}" src="Image.do?serverURL=${param.wadoUrl}&study=${param.study}&series=${seriesId}&object=${imageId}&rows=512" height="48px" />
                        </c:if>
                    </c:otherwise>
                </c:choose>
            </img:Image>
